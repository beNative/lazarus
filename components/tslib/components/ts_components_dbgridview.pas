...

{
  ...
  Changes by Tim Sinaeve:
     ...
     * Multiselection support : added by Tim Sinaeve:
       - IsRowMultiSelected
  ...
  TODO:
    - Add component editor to automatically create persistent columns for the
      connected fields.
    - remove support for older Delphi versions (versions before Delphi 4)
    - CTRL-A during MultiSelect selects all records

  Lazarus issues
    - Setting Editing to True does not cause the inplace editor to be shown. For
      an unknown reason a WM-exit message is triggered and this causes in
      combination with CancelOnExit that the Editing property goes back to False.
}

unit ts_Components_DBGridView;
...
uses
  Messages, SysUtils, CommCtrl, Classes, Controls, Graphics, Forms, StdCtrls,
  Math, ImgList, Dialogs, DB, DBCtrls,
...

{
  ...
  TDBGridRowMultiSelectEvent = procedure(Sender: TObject; Row: Integer; var Select : Boolean) of object;
...
begin
  ...
  //
  //FIndicatorsDef.;
  //ResInstLoad(HInstance, rtBitmap, 'BM_GRIDVIEW_DB', clFuchsia);
